 ...
 *
 */
public abstract class MEditableTextControl extends MTextBase {
	...
	public MEditableTextControl(PApplet theApplet, float p0, float p1, float p2, float p3, int scrollbars) {
		super(theApplet, p0, p1, p2, p3);
		scrollbarPolicy = scrollbars;
		autoHide = ((scrollbars & SCROLLBARS_AUTOHIDE) == SCROLLBARS_AUTOHIDE);
		caretFlasher = new MTimer(theApplet, this, "flashCaret", 400);
		caretFlasher.start();
		opaque = true;
		cursorOver = TEXT;
	}
	...
	protected void loseFocus(MAbstractControl grabber){
		...
		if(focusIsWith == this)
			fireEvent(this, MEvent.LOST_FOCUS);
	...
	protected void takeFocus(){
		...
		if(focusIsWith != this){
			...
			fireEvent(this, MEvent.GETS_FOCUS);
		}
		focusIsWith = this;
	}
	 ...
	 * This might be useful if you want to use a GTextArea control to render large
...
//		}
	 ...
	 */
	public void flashCaret(MTimer timer){
		showCaret = !showCaret;
	}
	 ...
	 */
	public void hsbEventHandler(MScrollbar scrollbar, MEvent event){
	 ...
	 */
	public void vsbEventHandler(MScrollbar scrollbar, MEvent event){
		keepCursorInView = false;
